...
#   pepatac conda env               ~5-8 GB (many bioinformatics tools + R/Bioc)
...
# the pepatac conda environment; wrapping it in conda run risks a false
...
# not part of the pepatac conda environment.
#
# The `|| echo ''` matters under set -e + pipefail (both active in this
...
if [[ -n "$R_PACKAGE_CHECK" ]]; then
    ...
    echo "    The R environment itself may be broken -- check the pepatac conda env directly."
fi
